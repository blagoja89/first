...
public class TrainDate extends Entity {
	...
	public DBProperty getDbProperty() {
		...
		dbProperty.getDbSimplePropertyItems().add(
				new DBSimplePropertyItem("id", "ID_TRAIN_DATE", Integer.TYPE));
		dbProperty.getDbSimplePropertyItems().add(
				new DBSimplePropertyItem("departureDate", "DEPARTURE_DATE",
						Date.class));
		dbProperty.getDbSimplePropertyItems().add(
				new DBSimplePropertyItem("departureTime", "DEPARTURE_TIME",
						Time.class));

		dbProperty.getDbManyToOnePropertyItems().add(
				new DBManyToOnePropertyItem("train", "TRAIN_ID", Train.class,
