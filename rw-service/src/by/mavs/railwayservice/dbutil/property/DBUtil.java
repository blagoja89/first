...
import by.mavs.railwayservice.dao.AbstractDao;
...
public class DBUtil {
	...
	private Object getValueByGetMethodCall(Entity entity, String fieldName, Class<?> fieldType, Class [] params) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException{
		Method m = entity.getClass().getMethod(
				dbQueryUtil.getMethodGetName(fieldName, fieldType), params);
		return m.invoke(entity, NO_PARAMS_OB);
	}
	
	private Object getValueByMethodCall(Entity entity, String fieldName, Class<?> fieldType, Class<?> noparams[]) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException{
		Object ob = null;
		Object noparamso[] = {};
		Method m = entity.getClass().getMethod(
				dbQueryUtil.getMethodGetName(fieldName, fieldType), noparams);
		ob = m.invoke(entity, noparamso);
		return ob;
	}
	
	public boolean executeMerge(Entity entity, PreparedStatement pstmt,
			boolean isNew) throws NoSuchMethodException, SecurityException,
			IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, SQLException {
		DBProperty dbProperty = entity.getDbProperty();
		int i = 0;
//		if(dbProperty.getDbExtendsPropertyItem() != null){
//			pstmt.setObject(++i, getValueByGetMethodCall(entity, dbProperty.getDbExtendsPropertyItem().getAppName(),  dbProperty.getDbExtendsPropertyItem().getAppClassName()));
//		}
	...
	public ResultSet findByProperty(Entity entity, PreparedStatement pstmt, String name, Object value)
			throws NoSuchMethodException, SecurityException,
			IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, SQLException {
...
//
//		if(dbProperty.getDbExtendsPropertyItem() != null){
//			Object ob = getValueByGetMethodCall(entity, dbProperty.getDbExtendsPropertyItem().getAppName(),
//					dbProperty.getDbExtendsPropertyItem().getAppClassName());
...
//			DBExtendsPropertyItem dbe = dbProperty.getDbExtendsPropertyItem();
//			if(dbe != null){
//				
//				Method mRs = RESULT_SET.getMethod(
//						dbQueryUtil.getMethodGetName(dbe.getAppClassName().getSimpleName(),dbe.getAppClassName()), new Class[]{String.class});
//				Object obRs = mRs.invoke(rs, dbe.getDbName());
//				Method m = entity.getClass().getMethod(
//						dbQueryUtil.getMethodSetName(dbe.getAppName()), new Class[]{dbe.getAppClassName()});
...
//			}
			for (DBSimplePropertyItem item : dbProperty.getDbSimplePropertyItems()) {
				...
				Method m = entity.getClass().getMethod(
						dbQueryUtil.getMethodSetName(item.getAppName()), new Class[]{type});
				m.invoke(entity, obRs);
			}
			...
			for (DBManyToOnePropertyItem item : dbProperty
				...
				Method m = entity.getClass().getMethod(
						dbQueryUtil.getMethodSetName(item.getAppName()), new Class[]{item.getClassName()});
				m.invoke(entity, ob);
			}

			for (DBOneToManyPropertyItem item : dbProperty
					.getDbOneToManyPropertyItems()) {
				...
				if(!item.isLazy()){
					...
					Method m = entity.getClass().getMethod(
							dbQueryUtil.getMethodSetName(item.getAppName()), new Class[]{List.class});
