...
<xsl:stylesheet version="1.0"
	...
	xmlns:validatorHandler="xalan://by.mavs.market.util.ValidatorHandler"
	xmlns:redirect="http://xml.apache.org/xalan/redirect">
	<xsl:import href="/MAVS/JAVA/workspace_mavs/market/src/main/resources/addProductForm.xsl" />
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:call-template name="addingPage">
					...
					<xsl:with-param name="modelError" select="validatorHandler:getModelError($validator)" />
					<xsl:with-param name="priceError" select="validatorHandler:getPriceError($validator)" />
					<xsl:with-param name="producerError" select="validatorHandler:getProducerError($validator)" />
					<xsl:with-param name="colorError" select="validatorHandler:getColorError($validator)" />
					<xsl:with-param name="dateError" select="validatorHandler:getDateError($validator)" />
				</xsl:call-template>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="addProduct"
		match="/market/category[@name=$catName]/subcategory[@name=$subcatName]">
